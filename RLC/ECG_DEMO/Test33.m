for demo =1
    ...
    ecgCropped=ecg(2447:2814);
    temp=correctedCropped+2399;
    y=fft(temp);
    
    ecg2C=ecg(29200:29567);
    k=fft(ecg2C);
    r=corrcoef(temp,ecg2C);
    g=corrcoef(y,k);
    temp2=corrcoef(ecgCropped,correctedCropped);
    disp('Correlation =');
    disp(r);
    disp(g);
    %disp(temp2);
    f = (0:length(correctedCropped)-1)*50/length(correctedCropped);
    j = (0:length(ecg2C)-1)*50/length(ecg2C);
    ...
    %xL = get(gca, 'XLim');
    %plot(xL, [0 0], '-')
    figure(demo+3); set(demo+3, 'Name', strcat(plotname, ' - Result'));
    
    plot(ecg2C);
    hold on
    plot(temp);
    figure(demo+4); set(demo+4, 'Name', strcat(plotname, ' - Result'));
